...
def info(user):
    ...
    if request.method == 'POST':
        if uform.currencys.data:
            ...
            return redirect(url_for('get_online',
                                    user=user))

        elif uform.get_history.data:
            currency = uform.curr_name.data
            if not currency:
                flash("Input Currency name or Symbol to search or choose from list !")
        ...
        elif uform.compare.data:
            c_list = request.form.getlist('check')
            choice = uform.choice.data

            if len(c_list) == 0:
                flash("Check some currency for compare !")

                return redirect(url_for('show_currencies',
                                        user=user))

            session['datas'] = c_list
...
@app.route('/info/<user>/history/<currency>/graphic', methods=['GET', 'POST'])
@check_login
def get_graphic(user, currency):
    uform = UserControlForm()
    api = session['api_id']
    ...
    dates = [date[7] for date in data]
    prices = [float(price[1].replace("$", "")) for price in data]

    return render_template('info/graphic.html',
                           user=user,
                           api=api,
                           currency=currency,
                           prices=prices,
...
@app.route('/info/<user>/online/', methods=['GET', 'POST'])
@check_login
def get_online(user):
    uform = UserControlForm()
    api = session['api_id']
    ...
    try:
        ...
        return render_template('info/online.html',
                               user=user,
                               data=data,
                               api=api,
                               uform=uform,
...
@app.route('/info/<user>/compare/<choice>/', methods=['GET', 'POST'])
@check_login
def compare(user, choice):
    uform = UserControlForm()
    api = session['api_id']
    datas = session['datas']
    ...
    column_list = {"pc": Info.price,
                   ...
                   "7d": Info.perc_7d}
    data = {}
    target = column_list[choice]

    for dt in datas:
        try:
            result = Currency.query.filter_by(name=dt.lower()).first()
            res_info = result.info.with_entities(Info.date, target).all()

        except Exception as err:
            flash("ERROR: Connecting error !")
            return redirect(url_for('index'))
        ...
        for row in res_info:
            date_range.append(row[0].strftime("%Y-%m-%d %H:%M:%S"))

            prices.append(float(row[1].replace("$", " ")
                                            .replace("%", " ")
                                            .replace(",", "")))

            data[dt.capitalize()] = prices

    print(choice)
    if choice in ["1h", "4h", "7d"]:
        pd.options.display.float_format = '{:,.2f}%'.format
    else:
        pd.options.display.float_format = '${:,.2f}'.format

    df = pd.DataFrame(data=data, index=date_range).round(2)[::-1]
    ...
    for dt in datas:
        ...
        rate_data[dt] = [min, max, mean]

    df_info = pd.DataFrame(data=rate_data,
                           index=['MIN', 'MAX', 'MEAN']).round(2)

    return render_template('info/compare.html',
                           user=user,
                           uform=uform,
                           data=df.to_html(classes="table table-inverse",
                                           border=0),
                           d_info=df_info.to_html(classes="table table-inverse",
                                                  border=0),
                           choice=choice,
                           api=api)


@app.route('/info/<user>/currencys/', methods=['GET', 'POST'])
@check_login
def show_currencies(user):
    uform = UserControlForm()
    api = session['api_id']
    ...
    return render_template('info/currencys.html',
                           user=user,
                           data=data,
                           api=api,
                           uform=uform)
...
@app.route('/info/<user>/history/<currency>', methods=['GET', 'POST'])
@check_login
def show_history(user, currency):
    uform = UserControlForm()
    api = session['api_id']
    ...
    if not currency:
        flash('Input Currency name or Symbol for search...')
    ...
    if not curr_query:
        flash('No such currency {0}: In Name or Symbol'.format(currency))
    ...
    for obj in history:
        data.append((obj.market_cap, obj.price, obj.cs,
                     ...
                     obj.perc_7d, obj.date.strftime("%Y-%m-%d %H:%M:%S"),))
...
def err404(e):
    ...
    """
    return render_template('err_pages/err404.html',
                           error=e)
...
def post_one(api_key):
    ...
    if not data:
        return jsonify({"ERROR": "No data to ADD!",
                        "Template:":
                            "name: Bitcoin, symbol: BTC, info: {}",
                        "Info params:":
                            "ms, price, cs, volume,"
                            "p1h, p24h, p7d, date_time",
                        "Data Format":
                            "JSON"})
    ...
    if 'info' not in data.keys():
        ...
        d_info = data['info']
        date_time = datetime.strptime(d_info['date_time'],
                                      "%Y-%m-%d %H:%M:%S")
